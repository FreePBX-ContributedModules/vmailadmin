...
$display			= (isset($_REQUEST["display"]) && $_REQUEST["display"] != "")?$_REQUEST["display"]:"vmailadmin";
...
if (isset($extens) && is_array($extens)) {
	...
	foreach ($extens as $key => $exten) {
		...
		$rnav_entries[$i] = "\t<li id='vmailadmin_list_" . $exten[0] . "'${disabled_style}${unactivated_style}><a" . ($extdisplay==$exten[0] ? ' class="current"':'') . "${disabled_style}${unactivated_style} href=\"$link\" onHover='menuUpdatePos();'>{$name} &lt;" . $exten[0] . "&gt;&nbsp;&nbsp;(${c}${disabled_txt})${unactivated_txt}</a></li>\n";
		$i++;
	}
}
...
if (isset($_REQUEST["ext"])) {
		...
		if (isset($vmail_info["activated_info"][$extension])) {
			...
			// Force vmailadmin to "system" mode by clearing context and extension values
			$context   = "";
			$extension = "";
		}
} else {
...
$rnav_menu = "<ul name='vmailadmin_menu' id='vmailadmin_menu' style='max-width:400px;'>\n" . $rnav_list . "</ul>";
$title	  = vmailadmin_get_title($action, $context, $extension);
$output   = "";
$output   .= "<div class='rnav'>$rnav_menu</div>";
$output   .= "<div class='content'>\n";
$output   .= "<form name='frm_vmailadmin' action='" . $_SERVER['PHP_SELF'] . "' method='post'>";
...
function find_in_menu(id) {
	...
	document.getElementById('vmailadmin_menu').scrollTop = objToFind.offsetTop - 2 * objToFind.offsetHeight;
}";
if ($extension != "") {
...
if ($need_update) {
	/* set args */
	$args = array();
	if (vmailadmin_update_settings($action, $context, $extension, $_REQUEST)) {
...
switch ($action) {
	case "tz":
		/* get tz settings */
		$settings = vmailadmin_get_settings($uservm, $action, $extension);
	...
	case "settings":
		...
		unset($settings["enabled"]);	# ignore this value; we will not enable/disable from vmailadmin
		if (is_array($settings) && !empty($settings)) {
			foreach ($settings as $key => $val) {
				$id = $id_prefix . "__" . $key;
	...
	case "usage":
		...
		*/
		$scope = vmailadmin_get_scope($extension);
		if ($need_update) {
			vmailadmin_update_usage($vmail_info, $context, $extension, $_REQUEST);
			if (!empty($extension)) {
		...
		vmailadmin_get_usage($vmail_info, $scope, $acts_total, $acts_act, $acts_unact, $disabled_count,
		...
		if ($scope == "system") {
			...
			$ts = vmailadmin_get_greeting_timestamps($name, $unavail, $busy, $temp, $context, $extension);
